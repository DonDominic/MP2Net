...
class COCO(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.img_dir0 = self.opt.data_dir   #/dataset/Skysat

        self.img_dir = self.opt.data_dir + '/' + split + '/'

        if split == 'train':
            self.annot_path = os.path.join(
                self.img_dir0, 'instances_{}_caronly.json').format(split)
        else:
            self.annot_path = os.path.join(
                self.img_dir0, 'instances_{}_caronly.json').format(split)
        ...
        print('==> initializing Skysat {} data.'.format(split))
    ...
    def __getitem__(self, index):
        ...
        file_name = self.coco.loadImgs(ids=[img_id])[0]['file_name']    # 001/img/000001.png
        ...
        imf = file_name.split(file_name.split('/')[-1])[0]      # 001/img/
        ...
        if self.aug is not None:
            ...
            sample_h, sample_w = (img.shape[0] // 32 + 1) * 32, (img.shape[0] // 32 + 1) * 32
            image_pad = np.zeros([sample_h, sample_w, 3, seq_num])
            for i in range(seq_num):
                image_pad[:,:,:,i] = cv2.copyMakeBorder(img[:,:,:,i], 0, max(0, sample_h - height), 0, max(0, sample_w - width), cv2.BORDER_CONSTANT, value=(0,0,0))
            img = image_pad
