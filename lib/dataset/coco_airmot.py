...
class COCO(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        if split == 'train':
            self.annot_path = os.path.join(
                self.img_dir0, 'instances_{}_2cate.json').format(split)
        else:
            self.annot_path = os.path.join(
                self.img_dir0, 'instances_{}_2cate.json').format(split)
        ...
        print('==> initializing AIRMOT {} data.'.format(split))
    ...
    def __getitem__(self, index):
        ...
        file_name = self.coco.loadImgs(ids=[img_id])[0]['file_name']    # video/1/img/000001.jpg
        ...
        imf = file_name.split(file_name.split('/')[-1])[0]      # video/1/img/
