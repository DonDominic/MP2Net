...
class ConvGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias=True, activation=F.tanh):
        ...
        self.conv_z = nn.Conv2d(in_channels=self.input_dim + self.hidden_dim * 2,
                              out_channels=self.hidden_dim * 2,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)
        
        self.conv_r = nn.Conv2d(in_channels=self.input_dim + self.hidden_dim * 2,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)

        self.conv_h11 = nn.Conv2d(in_channels=self.hidden_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)
        
        self.conv_h12 = nn.Conv2d(in_channels=self.input_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding,
                              bias=self.bias)
    ...
    def forward(self, input, l_prev, h_prev):
        combined = torch.cat((input, l_prev, h_prev), dim=1)  # concatenate along channel axis
        ...
        h_ = self.activation(self.conv_h11(input) + self.conv_h12(l_prev) + r * self.conv_h2(h_prev))
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.conv_h11.weight, gain=nn.init.calculate_gain('tanh'))
        self.conv_h11.bias.data.zero_()
        nn.init.xavier_uniform_(self.conv_h12.weight, gain=nn.init.calculate_gain('tanh'))
        self.conv_h12.bias.data.zero_()
...
class ConvGRU(nn.Module):
    ...
    def forward(self, input, l_prev, hidden_state=None):
        """
        Parameters
        ----------
        input_tensor:
        ...
        prev_layer = torch.unbind(l_prev, dim=int(self.batch_first))
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                h = self.cell_list[layer_idx](input=cur_layer_input[t], l_prev=prev_layer[t], h_prev=h)
