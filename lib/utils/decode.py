...
def ctdet_decode(heat, wh, reg=None, tracking=None, pre_inds=None, num_classes=1, cat_spec_wh=False, K=100):
    ...
    if cat_spec_wh:
      ...
      wh = wh.view(batch, K, 2)
    # if tracking is not None: # B, N-1, 2, H, W
    #   tracking_gather = _transpose_and_gather_feat(tracking[:, 0], pre_inds[0]).view(batch, K, 1, 2)
    #   for i in range(1, tracking.shape[1]):
    #     tracking_temp = _transpose_and_gather_feat(tracking[:, i], pre_inds[i]).view(batch, K, 1, 2)
    #     tracking_gather = torch.cat([tracking_gather, tracking_temp], dim=2)
    #   ret['tracking'] = tracking_gather
    ...
    ret['bboxes'] = bboxes  # 当前帧出来的topk: B, K, 2
    ret['scores'] = scores  # 当前帧出来的topk: B, K, 1
    ret['clses'] = clses
    ret['inds'] = inds  # 当前帧出来的topk: B, K
    ret['cts'] = torch.cat([xs, ys], dim=2) # 当前帧出来的topk: B, K, 2
    return ret
